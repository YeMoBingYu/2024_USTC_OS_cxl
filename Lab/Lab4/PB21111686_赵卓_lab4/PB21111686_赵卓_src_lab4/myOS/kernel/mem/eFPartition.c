...
unsigned long eFPartitionFree(unsigned long EFPHandler, unsigned long mbStart)
{
	...
	*/
	...
	// 判断mbStart是否在合法范围内
	if ((mbStart < EFPHandler + eFPartition_size) || (mbStart > EFPHandler + eFPartitionTotalSize(handle->perSize, handle->totalN)))
		return 0;
